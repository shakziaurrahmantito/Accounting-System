...
<div class="grid_10">
    ...
    <?php
       ...
       $sql = "UPDATE voucher SET
                    ...
                    ledger_id  = '$ledger_id',
                    sub_goup_id  = '$sub_goup_id',
                    phosting_head_id  = '$phosting_head_id',
                    ...
                    prepared_by = '$prepared_by' WHERE sl_no = '$update'";

               $result = $db->update($sql);
                if ($result) {
                   echo "<p style='color:green;text-align:center;'>Data update successfully.</p>";
                }else{
            ...
            $update = $_GET['update'];
                ...
                $usql = "SELECT 
                    ...
                    ledger_sub_group.ledger_sub_group_name,
                     ...
                     company.Company_name FROM

                    voucher INNER JOIN ledger_group
                    ...
                    INNER JOIN branch
                    ON voucher.branch_id = branch.Branch_id

                    INNER JOIN voucher_type
                    ON voucher.voucher_type = voucher_type.voucher_type_id 

                    INNER JOIN company
                    ON voucher.company_id = company.Company_id
                     WHERE voucher.sl_no = '$update'";
   ...
   ?>
        ...
        <?php
                ...
                if ($result) {
                while($row = $result->fetch_assoc()){
                    if ($row['ledger_id'] == $urow['ledger_id']) {
        ?>
        ...
        <?php
                $ledger_id = $urow['ledger_id'];

                $sql = "SELECT * FROM ledger_sub_group WHERE ledger_sub_group_parent_id = '$ledger_id' ORDER BY ledger_sub_group_name ASC";
                $result = $db->select($sql);
                if ($result) {
                while($row = $result->fetch_assoc()){
                    if ($row['ledger_sub_group_id'] == $urow['sub_goup_id']) {
        ?>
        ...
        <?php
                $sub_goup_id = $urow['sub_goup_id'];

                $sql = "SELECT * FROM ledger_posting_head WHERE ledger_sub_group_id = '$sub_goup_id' ORDER BY posting_head_name ASC";
                $result = $db->select($sql);
                if ($result) {
                while($row = $result->fetch_assoc()){
                if ($row['ledger_posting_head_id'] == $urow['phosting_head_id']) {
        ?>
        ...
        <?php
                ...
                if ($result) {
                while($row = $result->fetch_assoc()){
                    if ($row['voucher_type_id'] == $urow['voucher_type']) {
        ?>
                    <option selected value="<?php echo $row['voucher_type_id']; ?>"><?php echo $row['voucher_type_name']; ?></option>
        <?php 
                }else{
        ?>
        ...
        ?>
				...
				<tr>
                    ...
                    <td>
                        <input type="text" name="voucher_no" id="voucher_no"  value="<?php echo $urow['voucher_no']; ?>" readonly required class="medium"/>
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="debit_amount" id="debit_amount" class="medium" value="<?php echo $urow['debit_amount']; ?>" onkeyup="myEmpty()" onblur="myEmpty()" />
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" name="Credit_amount" value="<?php echo $urow['Credit_amount']; ?>" id="Credit_amount" class="medium" onkeyup="myEmpty()" onblur="myEmpty()" />
                ...
                <tr>
                    ...
                    <td>
                       <input type="text" name="check_no" id="check_no" value="<?php echo $urow['check_no']; ?>">
                ...
                <tr>
                    ...
                    <td>
                       <input type="date" name="check_date" id="check_date" value="<?php echo $urow['check_date']; ?>">
                ...
                <tr>
                    <td></td>
                    <td>
                        <input type="submit" name="Vupdate" Value="Update" />
                    </td>
                </tr>
            </table>
            </form>
