<?php include "inc/admin_header.php"; ?>
<?php include "inc/admin_sidebar.php"; ?>

<div class="grid_10">
    <div class="box round first grid">
        <h2>Add Ledger Group</h2>
       <div class="block copyblock">
        <?php

            if (isset($_POST['submit'])) {
                
                 if (!empty($_POST['ledger_name'])) {
                   $ledger_name       = trim($_POST['ledger_name']);
                    $sql            = "SELECT * FROM ledger_group WHERE ledger_name = '$ledger_name'";
                    $Checkledger_group  = $db->select($sql);
                }

                if(empty($_POST['ledger_name']) || empty($_POST['group_id'])) {
                    echo "<p style='color:red;text-align:center;'>Field must not be empty.</p>";
                }else if($Checkledger_group){
                     echo "<p style='color:red;text-align:center;'>Ledger name already exist.</p>";
                }else{

                    $ledger_name = $_POST['ledger_name'];
                    $group_id = $_POST['group_id'];
                    $sql = "INSERT INTO ledger_group(ledger_name,group_id) VALUES('$ledger_name','$group_id')"; 


                    $result = $db->insert($sql);
                    if ($result) {
                       echo "<p style='color:green;text-align:center;'>Data insert successfully.</p>";
                    }else{
                        echo "<p style='color:red;text-align:center;'>Data not inserted.</p>";
                    }
                }
            }
        ?>
         <form action="" method="post" id="myForm">
            <table class="form">
                <tr>
                    <td>
					    <label>Group Type</label>
                    </td>
                    <td>
                        <select name="group_id" id="group_id" onchange="emptValid(this.id,'errgroup_id','select')">
                           <option value="">Select group Type</option>
        <?php
                $sql = "SELECT * FROM group_type ORDER BY group_type_name ASC";
                $result = $db->select($sql);
                if ($result) {
                while($row = $result->fetch_assoc()){
        ?>
                    <option value="<?php echo $row['group_type_id']; ?>"><?php echo $row['group_type_name']; ?></option>
        <?php 
                }
            }
        ?>         
                    </select>
					<small style="display:block;" id="errgroup_id"></small>
                    </td>
                </tr>
				<tr>
                    <td>
						<label>Ledger Group Name</label>
                    </td>
                    <td>
                        <input type="text" name="ledger_name" id="ledger_name" onkeyup="emptValid(this.id,'errledger_name')" onblur="emptValid(this.id,'errledger_name')" placeholder="Enter ledger group name" class="medium" />
                        <small style="display:block;" id="errledger_name"></small>
                    </td>
                </tr>
                <tr>
                    <td></td>
                    <td>
                        <input type="submit" name="submit" Value="Save" />
                    </td>
                </tr>
            </table>
            </form>
        </div>
		<br>
        <hr>
        <table class="data display datatable" id="example">
            <thead>
                <tr>
                    <th width="20%">SL</th>        
                    <th width="30%">Group Name</th>
                    <th width="30%">Ledger Name</th>
                    <th width="20%">Action</th>
                </tr>
            </thead>
            <tbody>
                <?php 

                    $sql = "SELECT  
                    ledger_group.*,
                     group_type.group_type_name FROM
                    ledger_group INNER JOIN group_type
                    ON ledger_group.group_id = group_type.group_type_id ORDER BY ledger_group.ledger_name ASC";
                    $result = $db->select($sql);
                    if ($result) {
                    $i = 1;
                    while($row = $result->fetch_assoc()){
                ?>
                <tr>
                    <td><?php echo $i++; ?></td>
                    <td><?php echo $row['group_type_name']; ?></td>
                    <td><?php echo $row['ledger_name']; ?></td>
                    <td>
                        <a href="ledger_update.php?update=<?php echo $row['ledger_id']; ?>">Edit</a>
                    <?php
                        $role_id = Session::get("role_id");
                        if ($role_id == 1) {
                    ?> || 
                        <a onclick="return confirm('Are you sure to delete?')" href="delete.php?action=ledger_group&id=<?php echo $row['ledger_id']; ?>">Delete</a>
                    <?php
                        }
                    ?>
                    </td>
                </tr>
            <?php } } ?>

            </tbody>
        </table>
    </div>
</div>

<script type="text/javascript">
        
    $("#myForm").submit(function(){
        
		var group_id        = $("#group_id").val();
        var ledger_name     = $("#ledger_name").val();
		
		if ($.trim(group_id) == "") {
            $("#group_id").attr("style","border:2px solid red !important");
            $("#errgroup_id").html("Please select any option.").css("color","red");
            return false;
        }else{
             $("#errgroup_id").html("").css("color","");
        }

        if ($.trim(ledger_name) == "") {
            $("#ledger_name").attr("style","border:2px solid red !important");
            $("#errledger_name").html("Field must not be empty.").css("color","red");
            return false;

        }else if($.trim(ledger_name) !== ""){

           var check = "";

            $.ajax({
                async : false,
                url : "getExistData.php",
                data : {ledger_name:ledger_name},
                method : "post",
                dataType : "html",
                success : function(data){
                    if ($.trim(data) == "1") {
                        check = 1;
                    }
                }
            });

            if (check == "1") {
                $("#ledger_name").attr("style","border:2px solid red !important");
                $("#errledger_name").html("Data already exist.").css("color","red");
                return false;
            }else{
                $("#ledger_name").attr("style","border:");
                $("#errledger_name").html("").css("color","red");
            }
        }else{
             $("#errledger_name").html("").css("color","");
        }
		
    });
</script>

<script type="text/javascript">
        $(document).ready(function () {
            setupLeftMenu();

            $('.datatable').dataTable();
            setSidebarHeight();
        });
</script>
<?php include "inc/admin_footer.php"; ?>
